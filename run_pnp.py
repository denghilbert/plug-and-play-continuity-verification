...
def main():
    ...
    #for qk_threshold in range(49, 50, 5):
    qk_threshold = 0
    uncond_list = []
    ...
    for uncond_scale in uncond_list:
        ...
        print(f"uncond_scale is {uncond_scale}")
        outpaths = [os.path.join(f"{exp_path_root}/{exp_config.source_experiment_name}/translations_{opt.prompt}", f"{uncond_scale}_{translation_folder}") for translation_folder in translation_folders]
        out_label = f"{opt.prompt}_uncond_scale{uncond_scale}"

...
def main():
    ...
    if opt.experiment_name != "default_test":
        exp_config.config.experiment_name = opt.experiment_name
    ...
    for num_block in range(1, 12):
        print("block_layer" + str(num_block))
        if num_block == 6: continue
        for t in iterator:
            for feature_type in feature_types:
                if num_block < 3 and ('self_attn_q' in feature_type or 'self_attn_k' in feature_type): continue
                #fit_features = load_experiments_features(fit_feature_maps_paths, exp_config.config.block, feature_type, t)  # N X C
                fit_features = load_experiments_features(fit_feature_maps_paths, "output_block_" + str(num_block), feature_type, t)  # N X C
    ...
    for num_block in range(1, 12):
        for t in iterator:
            for feature_type in feature_types:
                if num_block < 3 and ('self_attn_q' in feature_type or 'self_attn_k' in feature_type): continue
                #fit_features = load_experiments_features(fit_feature_maps_paths, exp_config.config.block, feature_type, t)  # N X C
                fit_features = load_experiments_features(fit_feature_maps_paths, "input_block_" + str(num_block), feature_type, t)  # N X C
                transform_features = load_experiments_features(transform_feature_maps_paths, exp_config.config.block, feature_type, t)
                visualize_and_save_features_pca(torch.cat(fit_features, dim=0),
                                                torch.cat(transform_features, dim=0),
                                                transform_experiments,
